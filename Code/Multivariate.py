...
NDVI_df = pd.read_csv(os.path.join(upper_dir, "NDVI.tif.csv"), header=None)
NDVI_df = NDVI_df.replace(-32767,np.nan)
print(len(NDVI_df))

NDWI_df = pd.read_csv(os.path.join(upper_dir, "NDWI.tif.csv"),header=None)
NDWI_df = NDWI_df.replace(-32767,np.nan)
print(len(NDWI_df))

thermal_df = pd.read_csv(os.path.join(upper_dir, "thermal.tif.csv"),header=None)
thermal_df = thermal_df.replace(65535,np.nan)
print(len(thermal_df))

slope_df = pd.read_csv(os.path.join(upper_dir, "DEM_slope.tif.csv"),header=None)
slope_df = slope_df.replace(-9999.,np.nan)
print(len(slope_df))

roughness_df = pd.read_csv(os.path.join(upper_dir, "DEM_roughness.tif.csv"),header=None)
roughness_df = roughness_df.replace(-9999.,np.nan)
print(len(roughness_df))

aspect_df = pd.read_csv(os.path.join(upper_dir, "DEM_aspect.tif.csv"),header=None)
aspect_df = aspect_df.replace(-9999.,np.nan)
print(len(aspect_df))

hillshade_df = pd.read_csv(os.path.join(upper_dir, "DEM_hs.tif.csv"),header=None)
